...
Application: concatenation_test
Target solver: cvc5
Time limit: 0.1
...
;; concatenation Z
    (declare-fun |^ Z| (|POW (Z x Z)| |POW (Z x Z)|) |POW (Z x Z)|)
(assert (!
  (forall ((s1 |POW (Z x Z)|)(s2 |POW (Z x Z)|)(p |(Z x Z)|)) (!
    (= (|set.in (Z x Z)| p (|^ Z| s1 s2))
       (or (|set.in (Z x Z)| p s1)
           (|set.in (Z x Z)| (maplet (- (fst p) (|size Z| s1)) (snd p)) s2)))
    :pattern ( (|set.in (Z x Z)| p (|^ Z| s1 s2)) )))
  :named |ax.conc.definition Z|
))
(assert (!
    (not
      (= (|^ Z| (|set.intent (Z x Z)| (lambda ((_c0 |(Z x Z)|)) (or (= _c0 (maplet 1 2))(= _c0 (maplet 2 3))))) (|set.intent (Z x Z)| (lambda ((_c0 |(Z x Z)|)) (or (= _c0 (maplet 1 3))(= _c0 (maplet 2 4)))))) (|set.intent (Z x Z)| (lambda ((_c0 |(Z x Z)|)) (or (= _c0 (maplet 1 2))(= _c0 (maplet 2 3))(= _c0 (maplet 3 3))(= _c0 (maplet 4 4)))))))
    :named |Goal|))
(check-sat)
(exit)
