...
(assert (!
    (forall ((r1 |POW (Z x Z)|) (r2 |POW (Z x Z)|))
      (!
        (= (|rel.overwrite Z Z| r1 r2)
           (|set.intent (Z x Z)|
             (lambda ((x |(Z x Z)|))
               (or
                 (and (|set.in (Z x Z)| x r1)
                      (not (|set.in Z| (fst x) (|rel.domain Z Z| r2))))
                 (|set.in (Z x Z)| x r2)))))
        :pattern ((|rel.overwrite Z Z| r1 r2))))
...
(assert (!
    (not
      (= (|rel.overwrite Z Z| (|set.intent (Z x Z)| (lambda ((_c0 |(Z x Z)|)) (or (= _c0 (maplet 2 3))(= _c0 (maplet 3 4))))) (|set.intent (Z x Z)| (lambda ((_c0 |(Z x Z)|)) (= _c0 (maplet 2 4))))) (|set.intent (Z x Z)| (lambda ((_c0 |(Z x Z)|)) (or (= _c0 (maplet 2 4))(= _c0 (maplet 3 4)))))))
    :named |Goal|))
(check-sat)
(exit)
