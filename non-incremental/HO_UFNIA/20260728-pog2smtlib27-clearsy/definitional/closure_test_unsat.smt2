...
(assert (!(forall ((R |POW (Z x Z)|))
  (= (|closure1 Z| R) (|set.intent (Z x Z)| (lambda ((p |(Z x Z)|))
    (exists ((n |Z|)) (and (<= 1 n)
...
(assert (! (forall ((R |POW (Z x Z)|))
    (= (|closure Z| R) (|set.intent (Z x Z)| (lambda ((p |(Z x Z)|))
      (or (and (= (fst p) (snd p)) (|set.in Z| (fst p) (|rel.domain Z Z| R) )) (|set.in (Z x Z)| p (|closure1 Z| R)))))))
  :named |ax.closure Z|))
(assert (!
