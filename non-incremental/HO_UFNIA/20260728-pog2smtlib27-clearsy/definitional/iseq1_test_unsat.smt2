(set-info :smt-lib-version 2.7)
(set-logic HO_UFNIA)
(set-info :source "
Generated by: CLEARSY
Generated on: 2026-07-28
Generator: pog2smtlib-2.7
Application: iseq1_test
Target solver: cvc5
Time limit: 0.31
Benchmarks crafted to test the proof obligation translator pog2smtlib-2.7 for Atelier B.

MACHINE
input_unsat
ASSERTIONS
[1] : iseq1(INTEGER)
END
")
(set-info :license "https://creativecommons.org/licenses/by/4.0/")
(set-info :category "crafted")
(set-info :status unknown)

;; type Z
(define-sort |Z| () Int)
;; Cartesian product constructor
(declare-datatype C (par (T1 T2) ((maplet (fst T1) (snd T2)))))
;; Powerset constructor
(declare-sort P 1)
;; type (Z x Z)
(define-sort |(Z x Z)| () (C |Z| |Z|))
;; type POW Z
(define-sort |POW Z| () (P |Z|))
;; type ((Z x Z) x Z)
(define-sort |((Z x Z) x Z)| () (C |(Z x Z)| |Z|))
;; setmembership Z
(declare-fun |set.in Z| (|Z| |POW Z|) Bool)
;; type POW (Z x Z)
(define-sort |POW (Z x Z)| () (P |(Z x Z)|))
;; type POW ((Z x Z) x Z)
(define-sort |POW ((Z x Z) x Z)| () (P |((Z x Z) x Z)|))
;; setmembership (Z x Z)
(declare-fun |set.in (Z x Z)| (|(Z x Z)| |POW (Z x Z)|) Bool)
;; type POW POW ((Z x Z) x Z)
(define-sort |POW POW ((Z x Z) x Z)| () (P |POW ((Z x Z) x Z)|))
;; setmembership ((Z x Z) x Z)
(declare-fun |set.in ((Z x Z) x Z)| (|((Z x Z) x Z)| |POW ((Z x Z) x Z)|) Bool)
;; type POW POW (Z x Z)
(define-sort |POW POW (Z x Z)| () (P |POW (Z x Z)|))
;; setmembership POW ((Z x Z) x Z)
(declare-fun |set.in POW ((Z x Z) x Z)| (|POW ((Z x Z) x Z)| |POW POW ((Z x Z) x Z)|) Bool)
;; setmembership POW (Z x Z)
(declare-fun |set.in POW (Z x Z)| (|POW (Z x Z)| |POW POW (Z x Z)|) Bool)
;; ⊆ ((Z x Z) x Z)
(declare-fun |set.subseteq ((Z x Z) x Z)| (|POW ((Z x Z) x Z)| |POW ((Z x Z) x Z)|) Bool)
(assert (!
  (forall ((s |POW ((Z x Z) x Z)|) (t |POW ((Z x Z) x Z)|))
    (=
      (|set.subseteq ((Z x Z) x Z)| s t)
      (forall ((e |((Z x Z) x Z)|)) (=> (|set.in ((Z x Z) x Z)| e s) (|set.in ((Z x Z) x Z)| e t)))))
  :named |ax.set.subseteq ((Z x Z) x Z)|))
;; set POW ((Z x Z) x Z)
(define-sort |? POW ((Z x Z) x Z)| () (-> |POW ((Z x Z) x Z)| Bool))
(declare-const |set.intent POW ((Z x Z) x Z)| (-> |? POW ((Z x Z) x Z)| |POW POW ((Z x Z) x Z)|))
(assert (!
  (forall ((p |? POW ((Z x Z) x Z)|))
    (forall ((x |POW ((Z x Z) x Z)|))
      (= (|set.in POW ((Z x Z) x Z)| x (|set.intent POW ((Z x Z) x Z)| p))
         (@ p x))))
  :named |ax:set.in.intent POW ((Z x Z) x Z)|))
;; set ((Z x Z) x Z)
(define-sort |? ((Z x Z) x Z)| () (-> |((Z x Z) x Z)| Bool))
(declare-const |set.intent ((Z x Z) x Z)| (-> |? ((Z x Z) x Z)| |POW ((Z x Z) x Z)|))
(assert (!
  (forall ((p |? ((Z x Z) x Z)|))
    (forall ((x |((Z x Z) x Z)|))
      (= (|set.in ((Z x Z) x Z)| x (|set.intent ((Z x Z) x Z)| p))
         (@ p x))))
  :named |ax:set.in.intent ((Z x Z) x Z)|))
;; ⊆ (Z x Z)
(declare-fun |set.subseteq (Z x Z)| (|POW (Z x Z)| |POW (Z x Z)|) Bool)
(assert (!
  (forall ((s |POW (Z x Z)|) (t |POW (Z x Z)|))
    (=
      (|set.subseteq (Z x Z)| s t)
      (forall ((e |(Z x Z)|)) (=> (|set.in (Z x Z)| e s) (|set.in (Z x Z)| e t)))))
  :named |ax.set.subseteq (Z x Z)|))
;; set POW (Z x Z)
(define-sort |? POW (Z x Z)| () (-> |POW (Z x Z)| Bool))
(declare-const |set.intent POW (Z x Z)| (-> |? POW (Z x Z)| |POW POW (Z x Z)|))
(assert (!
  (forall ((p |? POW (Z x Z)|))
    (forall ((x |POW (Z x Z)|))
      (= (|set.in POW (Z x Z)| x (|set.intent POW (Z x Z)| p))
         (@ p x))))
  :named |ax:set.in.intent POW (Z x Z)|))
;; set (Z x Z)
(define-sort |? (Z x Z)| () (-> |(Z x Z)| Bool))
(declare-const |set.intent (Z x Z)| (-> |? (Z x Z)| |POW (Z x Z)|))
(assert (!
  (forall ((p |? (Z x Z)|))
    (forall ((x |(Z x Z)|))
      (= (|set.in (Z x Z)| x (|set.intent (Z x Z)| p))
         (@ p x))))
  :named |ax:set.in.intent (Z x Z)|))
;; set Z
(define-sort |? Z| () (-> |Z| Bool))
(declare-const |set.intent Z| (-> |? Z| |POW Z|))
(assert (!
  (forall ((p |? Z|))
    (forall ((x |Z|))
      (= (|set.in Z| x (|set.intent Z| p))
         (@ p x))))
  :named |ax:set.in.intent Z|))
;; POW ((Z x Z) x Z)
(declare-fun |sub-sets ((Z x Z) x Z)| (|POW ((Z x Z) x Z)|) |POW POW ((Z x Z) x Z)|)
(assert (!
(forall ((t |POW ((Z x Z) x Z)|)) (= (|sub-sets ((Z x Z) x Z)| t)
(|set.intent POW ((Z x Z) x Z)| (lambda ((s |POW ((Z x Z) x Z)|)) (|set.subseteq ((Z x Z) x Z)| s t)))))
  :named |ax.sub-sets ((Z x Z) x Z)|))
;; * (Z x Z) Z
(declare-fun |set.product (Z x Z) Z| (|POW (Z x Z)| |POW Z|) |POW ((Z x Z) x Z)|)
(assert (!(forall ((U |POW (Z x Z)|) (V |POW Z|))
    (= (|set.product (Z x Z) Z| U V) (|set.intent ((Z x Z) x Z)| (lambda ((p |((Z x Z) x Z)|))
           (and (|set.in (Z x Z)| (fst p) U)
                (|set.in Z| (snd p) V))))))
  :named |ax.set.product ((Z x Z) x Z)|))
;; = (Z x Z)
(assert (!
  (forall ((s |POW (Z x Z)|) (t |POW (Z x Z)|))
    (=
      (= s t)
      (forall ((e |(Z x Z)|)) (= (|set.in (Z x Z)| e s) (|set.in (Z x Z)| e t)))))
  :named |ax.set.eq (Z x Z)|))
;; = Z
(assert (!
  (forall ((s |POW Z|) (t |POW Z|))
    (=
      (= s t)
      (forall ((e |Z|)) (= (|set.in Z| e s) (|set.in Z| e t)))))
  :named |ax.set.eq Z|))
;; POW (Z x Z)
(declare-fun |sub-sets (Z x Z)| (|POW (Z x Z)|) |POW POW (Z x Z)|)
(assert (!
(forall ((t |POW (Z x Z)|)) (= (|sub-sets (Z x Z)| t)
(|set.intent POW (Z x Z)| (lambda ((s |POW (Z x Z)|)) (|set.subseteq (Z x Z)| s t)))))
  :named |ax.sub-sets (Z x Z)|))
;; * Z Z
(declare-fun |set.product Z Z| (|POW Z| |POW Z|) |POW (Z x Z)|)
(assert (!(forall ((U |POW Z|) (V |POW Z|))
    (= (|set.product Z Z| U V) (|set.intent (Z x Z)| (lambda ((p |(Z x Z)|))
           (and (|set.in Z| (fst p) U)
                (|set.in Z| (snd p) V))))))
  :named |ax.set.product (Z x Z)|))
;; ran (Z x Z) Z
(declare-fun |rel.range (Z x Z) Z| (|POW ((Z x Z) x Z)|) |POW Z|)
(assert (!
  (forall ((r |POW ((Z x Z) x Z)|))
  (= (|rel.range (Z x Z) Z| r) (|set.intent Z| (lambda ((e |Z|))
  (exists ((x |(Z x Z)|)) (|set.in ((Z x Z) x Z)| (maplet x e) r))))))
  :named |ax:set.in.range ((Z x Z) x Z)|
))
;; <-> (Z x Z) Z
(declare-fun |relations (Z x Z) Z| (|POW (Z x Z)| |POW Z|) |POW POW ((Z x Z) x Z)|)
(assert (!
  (forall ((X |POW (Z x Z)|) (Y |POW Z|))
    (= (|relations (Z x Z) Z| X Y)
       (|sub-sets ((Z x Z) x Z)| (|set.product (Z x Z) Z| X Y))))
    :named |def.relations ((Z x Z) x Z)|))
;; functions (Z x Z) Z
(declare-fun |functions (Z x Z) Z| (|POW (Z x Z)| |POW Z|) |POW POW ((Z x Z) x Z)|)
(assert (! 
  (forall ((X |POW (Z x Z)|) (Y |POW Z|))
      (= (|functions (Z x Z) Z| X Y)
        (|set.intent POW ((Z x Z) x Z)| (lambda ((f |POW ((Z x Z) x Z)|))
            (forall ((p |((Z x Z) x Z)|) (q |((Z x Z) x Z)|))
              (=> (and (|set.in ((Z x Z) x Z)| p f) (|set.in ((Z x Z) x Z)| q f) (= (fst p) (fst q)) )
                  (= (snd p) (snd q))))))))
    :named |ax:set.in.functions ((Z x Z) x Z)|))
;; dom (Z x Z) Z
(declare-fun |rel.domain (Z x Z) Z| (|POW ((Z x Z) x Z)|) |POW (Z x Z)|)
(assert (!
  (forall ((r |POW ((Z x Z) x Z)|))
    (= (|rel.domain (Z x Z) Z| r)
       (|set.intent (Z x Z)|
         (lambda ((e |(Z x Z)|))
           (exists ((y |Z|))
             (|set.in ((Z x Z) x Z)| (maplet e y) r))))))
             :named |ax:set.in.domain ((Z x Z) x Z)|))
;; ran Z Z
(declare-fun |rel.range Z Z| (|POW (Z x Z)|) |POW Z|)
(assert (!
  (forall ((r |POW (Z x Z)|))
  (= (|rel.range Z Z| r) (|set.intent Z| (lambda ((e |Z|))
  (exists ((x |Z|)) (|set.in (Z x Z)| (maplet x e) r))))))
  :named |ax:set.in.range (Z x Z)|
))
;; <-> Z Z
(declare-fun |relations Z Z| (|POW Z| |POW Z|) |POW POW (Z x Z)|)
(assert (!
  (forall ((X |POW Z|) (Y |POW Z|))
    (= (|relations Z Z| X Y)
       (|sub-sets (Z x Z)| (|set.product Z Z| X Y))))
    :named |def.relations (Z x Z)|))
;; functions Z Z
(declare-fun |functions Z Z| (|POW Z| |POW Z|) |POW POW (Z x Z)|)
(assert (! 
  (forall ((X |POW Z|) (Y |POW Z|))
      (= (|functions Z Z| X Y)
        (|set.intent POW (Z x Z)| (lambda ((f |POW (Z x Z)|))
            (forall ((p |(Z x Z)|) (q |(Z x Z)|))
              (=> (and (|set.in (Z x Z)| p f) (|set.in (Z x Z)| q f) (= (fst p) (fst q)) )
                  (= (snd p) (snd q))))))))
    :named |ax:set.in.functions (Z x Z)|))
;; dom Z Z
(declare-fun |rel.domain Z Z| (|POW (Z x Z)|) |POW Z|)
(assert (!
  (forall ((r |POW (Z x Z)|))
    (= (|rel.domain Z Z| r)
       (|set.intent Z|
         (lambda ((e |Z|))
           (exists ((y |Z|))
             (|set.in (Z x Z)| (maplet e y) r))))))
             :named |ax:set.in.domain (Z x Z)|))
;; intersection POW ((Z x Z) x Z)
(declare-fun |set.inter POW ((Z x Z) x Z)| (|POW POW ((Z x Z) x Z)| |POW POW ((Z x Z) x Z)|) |POW POW ((Z x Z) x Z)|)
(assert (!
(forall ((s |POW POW ((Z x Z) x Z)|) (t |POW POW ((Z x Z) x Z)|)) (= (|set.inter POW ((Z x Z) x Z)| s t)
(|set.intent POW ((Z x Z) x Z)| (lambda ((e |POW ((Z x Z) x Z)|)) (and (|set.in POW ((Z x Z) x Z)| e s) (|set.in POW ((Z x Z) x Z)| e t))))))
:named |ax.set.in.inter POW ((Z x Z) x Z)|))
;; surjections (Z x Z) Z
(declare-fun |surjections (Z x Z) Z| (|POW (Z x Z)| |POW Z|) |POW POW ((Z x Z) x Z)|)
(assert (!
  (forall ((X |POW (Z x Z)|) (Y |POW Z|))
      (= (|surjections (Z x Z) Z| X Y)
         (|set.intent POW ((Z x Z) x Z)|
         (lambda ((f |POW ((Z x Z) x Z)|))
           (= (|rel.range (Z x Z) Z| f) Y)))))
  :named |ax:set.in.surjections ((Z x Z) x Z)|))
;; injections (Z x Z) Z
(declare-fun |injections (Z x Z) Z| (|POW (Z x Z)| |POW Z|) |POW POW ((Z x Z) x Z)|)
(assert (!
  (forall ((X |POW (Z x Z)|) (Y |POW Z|))
    (= (|injections (Z x Z) Z| X Y)
       (|set.intent POW ((Z x Z) x Z)|
         (lambda ((f |POW ((Z x Z) x Z)|))
           (forall ((p |((Z x Z) x Z)|) (q |((Z x Z) x Z)|))
             (=> (and (|set.in ((Z x Z) x Z)| p f) (|set.in ((Z x Z) x Z)| q f) (= (snd p) (snd q)))
                 (= (fst p) (fst q))))))))
  :named |ax:set.in.injections ((Z x Z) x Z)|))
;; +-> (Z x Z) Z
(declare-fun |functions.partial (Z x Z) Z| (|POW (Z x Z)| |POW Z|) |POW POW ((Z x Z) x Z)|)
(assert (! 
  (forall ((e1 |POW (Z x Z)|) (e2 |POW Z|))
    (= (|functions.partial (Z x Z) Z| e1 e2) (|set.intent POW ((Z x Z) x Z)|
  (lambda ((f |POW ((Z x Z) x Z)|)) (and
  (|set.in POW ((Z x Z) x Z)| f (|relations (Z x Z) Z| e1 e2))
  (|set.in POW ((Z x Z) x Z)| f (|functions (Z x Z) Z| e1 e2)))))))
    :named |ax:def.pfun ((Z x Z) x Z)|))
;; >->> (Z x Z) Z
(declare-fun |relations.total (Z x Z) Z| (|POW (Z x Z)| |POW Z|) |POW POW ((Z x Z) x Z)|)
(assert (!
  (forall ((X |POW (Z x Z)|) (Y |POW Z|))
    (= (|relations.total (Z x Z) Z| X Y)
       (|set.intent POW ((Z x Z) x Z)|
         (lambda ((f |POW ((Z x Z) x Z)|))
           (= (|rel.domain (Z x Z) Z| f) X)))))
  :named |ax:set.in.relations.total ((Z x Z) x Z)|
))
;; intersection POW (Z x Z)
(declare-fun |set.inter POW (Z x Z)| (|POW POW (Z x Z)| |POW POW (Z x Z)|) |POW POW (Z x Z)|)
(assert (!
(forall ((s |POW POW (Z x Z)|) (t |POW POW (Z x Z)|)) (= (|set.inter POW (Z x Z)| s t)
(|set.intent POW (Z x Z)| (lambda ((e |POW (Z x Z)|)) (and (|set.in POW (Z x Z)| e s) (|set.in POW (Z x Z)| e t))))))
:named |ax.set.in.inter POW (Z x Z)|))
;; surjections Z Z
(declare-fun |surjections Z Z| (|POW Z| |POW Z|) |POW POW (Z x Z)|)
(assert (!
  (forall ((X |POW Z|) (Y |POW Z|))
      (= (|surjections Z Z| X Y)
         (|set.intent POW (Z x Z)|
         (lambda ((f |POW (Z x Z)|))
           (= (|rel.range Z Z| f) Y)))))
  :named |ax:set.in.surjections (Z x Z)|))
;; injections Z Z
(declare-fun |injections Z Z| (|POW Z| |POW Z|) |POW POW (Z x Z)|)
(assert (!
  (forall ((X |POW Z|) (Y |POW Z|))
    (= (|injections Z Z| X Y)
       (|set.intent POW (Z x Z)|
         (lambda ((f |POW (Z x Z)|))
           (forall ((p |(Z x Z)|) (q |(Z x Z)|))
             (=> (and (|set.in (Z x Z)| p f) (|set.in (Z x Z)| q f) (= (snd p) (snd q)))
                 (= (fst p) (fst q))))))))
  :named |ax:set.in.injections (Z x Z)|))
;; +-> Z Z
(declare-fun |functions.partial Z Z| (|POW Z| |POW Z|) |POW POW (Z x Z)|)
(assert (! 
  (forall ((e1 |POW Z|) (e2 |POW Z|))
    (= (|functions.partial Z Z| e1 e2) (|set.intent POW (Z x Z)|
  (lambda ((f |POW (Z x Z)|)) (and
  (|set.in POW (Z x Z)| f (|relations Z Z| e1 e2))
  (|set.in POW (Z x Z)| f (|functions Z Z| e1 e2)))))))
    :named |ax:def.pfun (Z x Z)|))
;; >->> Z Z
(declare-fun |relations.total Z Z| (|POW Z| |POW Z|) |POW POW (Z x Z)|)
(assert (!
  (forall ((X |POW Z|) (Y |POW Z|))
    (= (|relations.total Z Z| X Y)
       (|set.intent POW (Z x Z)|
         (lambda ((f |POW (Z x Z)|))
           (= (|rel.domain Z Z| f) X)))))
  :named |ax:set.in.relations.total (Z x Z)|
))
;; bijections (Z x Z) Z
(declare-fun |bijections (Z x Z) Z| (|POW (Z x Z)| |POW Z|) |POW POW ((Z x Z) x Z)|)
  (assert (!
  (forall ((X |POW (Z x Z)|) (Y |POW Z|))
    (= (|bijections (Z x Z) Z| X Y)
       (|set.inter POW ((Z x Z) x Z)|
         (|injections (Z x Z) Z| X Y)
         (|surjections (Z x Z) Z| X Y))))
  :named |ax:set.in.bijections ((Z x Z) x Z)|))
;; --> (Z x Z) Z
(declare-fun |functions.total (Z x Z) Z| (|POW (Z x Z)| |POW Z|) |POW POW ((Z x Z) x Z)|)
(assert (!
  (forall ((e1 |POW (Z x Z)|) (e2 |POW Z|))
    (= (|functions.total (Z x Z) Z| e1 e2)
       (|set.intent POW ((Z x Z) x Z)|
         (lambda ((f |POW ((Z x Z) x Z)|))
           (and (|set.in POW ((Z x Z) x Z)| f (|functions.partial (Z x Z) Z| e1 e2))
            (|set.in POW ((Z x Z) x Z)| f (|relations.total (Z x Z) Z| e1 e2)))))))
  :named |ax:def.tfun ((Z x Z) x Z)|))
;; bijections Z Z
(declare-fun |bijections Z Z| (|POW Z| |POW Z|) |POW POW (Z x Z)|)
  (assert (!
  (forall ((X |POW Z|) (Y |POW Z|))
    (= (|bijections Z Z| X Y)
       (|set.inter POW (Z x Z)|
         (|injections Z Z| X Y)
         (|surjections Z Z| X Y))))
  :named |ax:set.in.bijections (Z x Z)|))
;; --> Z Z
(declare-fun |functions.total Z Z| (|POW Z| |POW Z|) |POW POW (Z x Z)|)
(assert (!
  (forall ((e1 |POW Z|) (e2 |POW Z|))
    (= (|functions.total Z Z| e1 e2)
       (|set.intent POW (Z x Z)|
         (lambda ((f |POW (Z x Z)|))
           (and (|set.in POW (Z x Z)| f (|functions.partial Z Z| e1 e2))
            (|set.in POW (Z x Z)| f (|relations.total Z Z| e1 e2)))))))
  :named |ax:def.tfun (Z x Z)|))
;; >->> (Z x Z) Z
(declare-fun |bijections.total (Z x Z) Z| (|POW (Z x Z)| |POW Z|) |POW POW ((Z x Z) x Z)|)
(assert(!
(forall ((e1 |POW (Z x Z)|) (e2 |POW Z|))
    (= (|bijections.total (Z x Z) Z| e1 e2)
       (|set.inter POW ((Z x Z) x Z)|
         (|functions.total (Z x Z) Z| e1 e2)
         (|bijections (Z x Z) Z| e1 e2))))
  :named |ax:def.tbij ((Z x Z) x Z)|))
;; Cardinals constructor
(declare-datatype Cardinals ( ( Infinite ) ( Finite ( Value Int ) )))
;; ..
(declare-fun |interval| (|Z| |Z|) |POW Z|)
(assert (!
  (forall ((l |Z|)(u |Z|))
    (= (|interval| l u)
      (|set.intent Z| (lambda ((e |Z|)) (and (<= l e) (<= e u))))))
  :named |ax.set.in.interval|))
;; >->> Z Z
(declare-fun |bijections.total Z Z| (|POW Z| |POW Z|) |POW POW (Z x Z)|)
(assert(!
(forall ((e1 |POW Z|) (e2 |POW Z|))
    (= (|bijections.total Z Z| e1 e2)
       (|set.inter POW (Z x Z)|
         (|functions.total Z Z| e1 e2)
         (|bijections Z Z| e1 e2))))
  :named |ax:def.tbij (Z x Z)|))
;; ⊆ POW (Z x Z)
(declare-fun |set.subseteq POW (Z x Z)| (|POW POW (Z x Z)| |POW POW (Z x Z)|) Bool)
(assert (!
  (forall ((s |POW POW (Z x Z)|) (t |POW POW (Z x Z)|))
    (=
      (|set.subseteq POW (Z x Z)| s t)
      (forall ((e |POW (Z x Z)|)) (=> (|set.in POW (Z x Z)| e s) (|set.in POW (Z x Z)| e t)))))
  :named |ax.set.subseteq POW (Z x Z)|))
;; card (Z x Z)
(declare-fun |card (Z x Z)| (|POW (Z x Z)|) Cardinals)
  (assert (!
  (forall ((s |POW (Z x Z)|) (n Z))
    (= (= (Value (|card (Z x Z)| s)) n)
        (exists ((f |POW ((Z x Z) x Z)|))
          (|set.in POW ((Z x Z) x Z)| f (|bijections.total (Z x Z) Z| s (|interval| 1 n))))))
  :named |ax.card.definition (Z x Z)|))
(assert (!
  (forall ((s |POW (Z x Z)|))
    (= (= (|card (Z x Z)| s) Infinite)
       (forall ((n Z))
         (not (exists ((f |POW ((Z x Z) x Z)|))
           (|set.in POW ((Z x Z) x Z)| f (|bijections.total (Z x Z) Z| s (|interval| 1 n))))))))
  :named |ax.card.definition1 (Z x Z)|))
;; card Z
(declare-fun |card Z| (|POW Z|) Cardinals)
  (assert (!
  (forall ((s |POW Z|) (n Z))
    (= (= (Value (|card Z| s)) n)
        (exists ((f |POW (Z x Z)|))
          (|set.in POW (Z x Z)| f (|bijections.total Z Z| s (|interval| 1 n))))))
  :named |ax.card.definition Z|))
(assert (!
  (forall ((s |POW Z|))
    (= (= (|card Z| s) Infinite)
       (forall ((n Z))
         (not (exists ((f |POW (Z x Z)|))
           (|set.in POW (Z x Z)| f (|bijections.total Z Z| s (|interval| 1 n))))))))
  :named |ax.card.definition1 Z|))
;; {} (Z x Z)
(declare-const |set.empty (Z x Z)| |POW (Z x Z)|)
(assert (! 
(= |set.empty (Z x Z)| (|set.intent (Z x Z)| (lambda ((e |(Z x Z)|)) false)))
  :named |ax.set.in.empty (Z x Z)|))
;; Natural1 constructor
(declare-const NATURAL1 |POW Z|)
(assert (!
  (= NATURAL1 (|set.intent Z| (lambda ((e |Z|)) (<= 1 e)))) :named |ax.set.in.NATURAL1|))
;; seq Z
(declare-fun |seq Z| (|POW Z|) |POW POW (Z x Z)|)
(assert (!
  (forall ((E |POW Z|) (s |POW (Z x Z)|))
    (=>
      (|set.in POW (Z x Z)| s (|seq Z| E))
      ((_ is Finite) (|card (Z x Z)| s))))
  :named |ax.seq.is.finite Z|))
(assert (!
  (forall ((E |POW Z|))
    (|set.subseteq POW (Z x Z)|
      (|seq Z| E)
      (|functions.total Z Z| (|interval| 1 (Value (|card Z| E))) E)))
  :named |ax.seq Z|))
;; [] Z

(declare-const |seq.empty Z| |POW (Z x Z)|)
(assert (!
  (= |seq.empty Z| |set.empty (Z x Z)|)
  :named |ax:seq.empty.def Z|))
;; iseq Z
(declare-fun |iseq Z| (|POW Z|) |POW POW (Z x Z)|)
(assert (!
(forall ((E |POW Z|)) 
  (= (|iseq Z| E)
    (|set.intent POW (Z x Z)| (lambda ((s |POW (Z x Z)|)) (and (|set.in POW (Z x Z)| s (|seq Z| E))
    (|set.in POW (Z x Z)| s (|injections Z Z| NATURAL1 E)))))))
      :named |ax.iseq Z|))
;; iseq1 Z
(declare-fun |iseq1 Z| (|POW Z|) |POW POW (Z x Z)|)
(assert (! 
(forall ((E |POW Z|)) (= (|iseq1 Z| E)
(|set.intent POW (Z x Z)| (lambda ((s |POW (Z x Z)|)) (and (|set.in POW (Z x Z)| s (|iseq Z| E))
(not (= s |seq.empty Z|)))))))
  :named |ax.iseq1 Z|))
;; Integer constructor
;; universe Z
(declare-const INTEGER |POW Z|)
(assert (!
  (= INTEGER (|set.intent Z| (lambda ((e |Z|)) true) ))
  :named |ax.rw.universe Z|))
(assert (!
    (not
      (|set.in POW (Z x Z)| (|set.intent (Z x Z)| (lambda ((_c0 |(Z x Z)|)) (= _c0 (maplet 1 1)))) (|iseq1 Z| INTEGER)))
    :named |Goal|))
(check-sat)
(exit)
