...
;; {} ((Z x Z) x (Z x Z))
(declare-const |set.empty ((Z x Z) x (Z x Z))| |POW ((Z x Z) x (Z x Z))|)
(assert (!
  (forall ((e |((Z x Z) x (Z x Z))|)) (!
    (not (|set.in ((Z x Z) x (Z x Z))| e |set.empty ((Z x Z) x (Z x Z))|))
    :pattern ((|set.in ((Z x Z) x (Z x Z))| e |set.empty ((Z x Z) x (Z x Z))|))))
  :named |ax.set.in.empty ((Z x Z) x (Z x Z))|))
;; = ((Z x Z) x (Z x Z))
(assert (!
...
(assert (!
    (not
      (= (|parallelproduct Z Z Z Z| (|set.intent (Z x Z)| (lambda ((_c0 |(Z x Z)|)) (or (= _c0 (maplet 2 3))(= _c0 (maplet 3 4))))) (|set.intent (Z x Z)| (lambda ((_c0 |(Z x Z)|)) (or (= _c0 (maplet 2 3))(= _c0 (maplet 3 4)))))) |set.empty ((Z x Z) x (Z x Z))|))
    :named |Goal|))
(check-sat)
(exit)
