(set-info :smt-lib-version 2.7)
(set-logic HO_UFNIA)
(set-info :source "
Generated by: CLEARSY
Generated on: 2026-07-28
Generator: pog2smtlib-2.7
Application: iterate_test
Target solver: cvc5
Time limit: 0.31
Benchmarks crafted to test the proof obligation translator pog2smtlib-2.7 for Atelier B.

MACHINE
input_unsat
ASSERTIONS
iterate({(1 |-> 2),(2 |-> 3)},2) = {(1 |-> 2),(2 |-> 3),(1 |-> 3)}
END
")
(set-info :license "https://creativecommons.org/licenses/by/4.0/")
(set-info :category "crafted")
(set-info :status unknown)

;; type Z
(define-sort |Z| () Int)
;; Powerset constructor
(declare-sort P 1)
;; Cartesian product constructor
(declare-datatype C (par (T1 T2) ((maplet (fst T1) (snd T2)))))
;; type POW Z
(define-sort |POW Z| () (P |Z|))
;; type (Z x Z)
(define-sort |(Z x Z)| () (C |Z| |Z|))
;; setmembership Z
(declare-fun |set.in Z| (|Z| |POW Z|) Bool)
;; type POW (Z x Z)
(define-sort |POW (Z x Z)| () (P |(Z x Z)|))
;; setmembership (Z x Z)
(declare-fun |set.in (Z x Z)| (|(Z x Z)| |POW (Z x Z)|) Bool)
;; set (Z x Z)
(define-sort |? (Z x Z)| () (-> |(Z x Z)| Bool))
(declare-const |set.intent (Z x Z)| (-> |? (Z x Z)| |POW (Z x Z)|))
(assert (!
  (forall ((p |? (Z x Z)|) (x |(Z x Z)|)) (!
    (= (|set.in (Z x Z)| x (|set.intent (Z x Z)| p))
       (@ p x))
    :pattern ((|set.in (Z x Z)| x (|set.intent (Z x Z)| p)))))
  :named |ax.rw.intent (Z x Z)|))
;; set Z
(define-sort |? Z| () (-> |Z| Bool))
(declare-const |set.intent Z| (-> |? Z| |POW Z|))
(assert (!
  (forall ((p |? Z|) (x |Z|)) (!
    (= (|set.in Z| x (|set.intent Z| p))
       (@ p x))
    :pattern ((|set.in Z| x (|set.intent Z| p)))))
  :named |ax.rw.intent Z|))
;; id Z
(declare-fun |id Z| (|POW Z|) |POW (Z x Z)|)
(assert (!
    (forall ((X |POW Z|) (x |(Z x Z)|))
      (!
        (= (|set.in (Z x Z)| x (|id Z| X))
           (and (|set.in Z| (fst x) X)
                (= (fst x) (snd x))))
        :pattern ((|set.in (Z x Z)| x (|id Z| X)))))
    :named |def.id Z|))
;; dom Z Z
(declare-fun |rel.domain Z Z| (|POW (Z x Z)|) |POW Z|)
(assert (!
  (forall ((r |POW (Z x Z)|) (e |Z|)) (!
    (= (|set.in Z| e (|rel.domain Z Z| r))
       (exists ((y |Z|)) (|set.in (Z x Z)| (maplet e y) r)))
    :pattern ((|set.in Z| e (|rel.domain Z Z| r)))))
  :named |ax:set.in.domain (Z x Z)|))
;; composition Z Z Z
(declare-fun |relcomp Z Z Z| (|POW (Z x Z)| |POW (Z x Z)|) |POW (Z x Z)|)
(assert (!
  (forall ((r |POW (Z x Z)|) (s |POW (Z x Z)|) (p |(Z x Z)|))
    (!
      (= (|set.in (Z x Z)| p (|relcomp Z Z Z| r s))
         (exists ((y |Z|))
           (and
             (|set.in (Z x Z)| (maplet (fst p) y) r)
             (|set.in (Z x Z)| (maplet y (snd p)) s))))
      :pattern ((|set.in (Z x Z)| p (|relcomp Z Z Z| r s)))))
  :named |ax.set.in.relcomp ((Z x Z) x Z)|))
;; iterate Z
(declare-fun |iterate Z| (|POW (Z x Z)| |Z|) |POW (Z x Z)|)
(assert (!
  (forall ((R |POW (Z x Z)|)) (!
    (= (|iterate Z| R 1) R)
    :pattern ((|iterate Z| R 1))))
  :named |ax.set.iterate.1 Z|))
(assert (!
  (forall ((R |POW (Z x Z)|) (n |Z|))
    (!
      (= (|iterate Z| R (+ n 1)) (|relcomp Z Z Z| R (|iterate Z| R n)))
      :pattern ((|iterate Z| R (+ n 1)))))
  :named |ax.set.iterate.n+1 Z|))
;; = (Z x Z)
(assert (!
  (forall ((s |POW (Z x Z)|) (t |POW (Z x Z)|))
    (=
      (= s t)
      (forall ((e |(Z x Z)|)) (= (|set.in (Z x Z)| e s) (|set.in (Z x Z)| e t)))))
  :named |ax.set.eq (Z x Z)|))
(assert (!
    (not
      (= (|iterate Z| (|set.intent (Z x Z)| (lambda ((_c0 |(Z x Z)|)) (or (= _c0 (maplet 1 2))(= _c0 (maplet 2 3))))) 2) (|set.intent (Z x Z)| (lambda ((_c0 |(Z x Z)|)) (or (= _c0 (maplet 1 2))(= _c0 (maplet 2 3))(= _c0 (maplet 1 3)))))))
    :named |Goal|))
(check-sat)
(exit)
