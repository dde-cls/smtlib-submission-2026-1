...
(assert (!
  (forall ((E |POW Z|) (s |POW (Z x Z)|)) (!
    ...
    :pattern ((|set.in POW (Z x Z)| s (|iseq Z| E))))
  )
  :named |ax.iseq Z|))
;; iseq1 Z
...
(assert (!
  (forall ((E |POW Z|) (s |POW (Z x Z)|))
    (!
      (= (|set.in POW (Z x Z)| s (|iseq1 Z| E))
         (and (|set.in POW (Z x Z)| s (|iseq Z| E))
              (not (= s |seq.empty Z|))))
      :pattern ((|set.in POW (Z x Z)| s (|iseq1 Z| E)))))
  :named |ax.iseq1 Z|))
;; Integer constructor
;; universe Z
...
(assert (!
    (not
      (|set.in POW (Z x Z)| (|set.intent (Z x Z)| (lambda ((_c0 |(Z x Z)|)) (= _c0 (maplet 1 1)))) (|iseq1 Z| INTEGER)))
    :named |Goal|))
(check-sat)
(exit)
