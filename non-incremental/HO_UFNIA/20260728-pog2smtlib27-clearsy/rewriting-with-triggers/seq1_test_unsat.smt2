  ...
  (forall ((E |POW Z|) (s |POW (Z x Z)|))
    (! (= (|set.in POW (Z x Z)| s (|seq1 Z| E))
          (and (|set.in POW (Z x Z)| s (|seq Z| E))
               (not (= s |seq.empty Z|))))
       :pattern ((|set.in POW (Z x Z)| s (|seq1 Z| E)))))
  :named |ax.seq1 Z|))
(assert (!
    (not
      (|set.in POW (Z x Z)| (|set.intent (Z x Z)| (lambda ((_c0 |(Z x Z)|)) (= _c0 (maplet 1 1)))) (|seq1 Z| (|set.intent Z| (lambda ((_c0 |Z|)) (or (= _c0 1)(= _c0 2)(= _c0 3)))))))
    :named |Goal|))
(check-sat)
(exit)
